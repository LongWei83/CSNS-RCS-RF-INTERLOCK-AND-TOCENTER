 ...
 *
 * drvNetMpf_314: A common EPICS device support for network-based devices
 * (R3.14)
...
#define min(x,y) ({typeof(x) _x = (x); typeof(y) _y = (y); (void)(&_x == &y);_x < _y? _x : _y; })
...
LOCAL long prepare_socket(MPF_COMMON *);
 ...
 */
LOCAL long prepare_socket(MPF_COMMON *m)
{
#ifdef vxWorks
  ...
  LOGMSG("drvNetMpf: prepare_socket(0x%08x)\n",m,0,0,0,0,0,0,0,0);
  ...
  if (isUdp(m->option))
    {
      ...
      len = recvfrom(
		     m->sfd,
		     cur,
		     RECV_BUF_SIZE(m->option),
		     0,
		     (struct sockaddr *) &m->sender_addr,
		     &sender_addr_len
		     );
      if ((m->recv_len > 0 && len != m->recv_len) || len == ERROR)
	{
	  errlogPrintf("drvNetMpf: recvfrom() error [%d], expected:%d, gotten:%d",
		       errno, m->recv_len, len);
	  return ERROR;
	}
      else
	{
	...
	len = recv(
		   m->sfd,
		   (char *) cur,
		   m->recv_len ? min(end - cur, m->recv_len) : end - cur,
		   0
		   );
	if (len == ERROR)
	  {
